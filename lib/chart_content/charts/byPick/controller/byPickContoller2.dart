...
class byPickController2 extends GetxController{
 ...
 // 각 번호의 당첨 횟수를 전체 추첨 횟수로 나누어 계산합니다.
  ...
  Map<int, double> calculateProbabilitiesMethod1(List<int> userNumbers) {
  ...
  Map<int, double> calculateProbabilitiesMethod2(List<int> userNumbers) {
    final numberFrequencies = calculateNumberFrequencies();
  ...
  //
  //
  // Map<int, double> calculatePercentages() {
  //   int totalOccurrences = numberOccurrences.values.reduce((a, b) => a + b);
  //   return numberOccurrences.map((number, count) =>
  //       MapEntry(number, (count / totalOccurrences) * 100));
  // }
